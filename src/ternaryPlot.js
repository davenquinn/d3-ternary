...
export default function ternaryPlot(ternary) {
  ...
  ternaryPlot.axisLabels = function () {
    return [A, B, C].map((d) => ({
      position: [
        d.vertex[0] * (radius + d.labelOffset),
        d.vertex[1] * (radius + d.labelOffset),
      ],
      label: d.label,
  ...
  ternaryPlot.domains = function (_) {
    ...
    const domainTransform = ternaryPlot.transformFromDomains(_);

    // Apply transfrom (ie update barycentric coord system)
  ...
  ternaryPlot.tickCounts = function (_) {
    return arguments.length
      ? !Array.isArray(_)
        ? ((A.tickCount = B.tickCount = C.tickCount = +_), ternaryPlot)
        : ((A.tickCount = _[0]),
          (B.tickCount = _[1]),
          (C.tickCount = _[2]),
          ternaryPlot)
  ...
  ternaryPlot.transform = function (transform) {
    ...
    const [newvA, newvB] = vertices.map(([vx, vy]) => [
      vx * k + tx,
      vy * k + ty,
    ]); // these are the vertices BEFORE checking if they within bounds of original triangle
    ...
    const transformedVertices = vertices.map(([vx, vy]) => [
      vx * k + tx,
      vy * k + ty,
  ...
  ternaryPlot.domainsFromVertices = function (ternary) {
    ...
    // setDomains here or let user handle that?
