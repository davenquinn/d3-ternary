...
[Try d3-ternary in your browser on Observable](https://observablehq.com/@julesblm/introducing-d3-ternary-plot?collection=@julesblm/ternary-plots)
...
[#](#baryCentricADoc) *barycentric*.**a**([_a_]) [<>](todo)
...
[#](#baryCentricBDoc) *barycentric*.**b**([_b_]) [<>](todo)
...
[#](#baryCentricCDoc) *barycentric*.**c**([_c_]) [<>](todo)
...
[#](#baryCentricConvertDoc) *barycentric*.**convert**(_data_) [<>](todo)
...
[#](#baryCentricInvertDoc) *barycentric*.**invert**(_coordinates_) [<>](todo)
...
[#](#baryCentricNormalizeDoc) *barycentric*.**normalize**(_ternaryData_) [<>](todo)

Computes normalized ternary values by summing and taking proportions of ternary data using the value accessors.

[#](#baryCentricVerticesDoc) *barycentric*.**vertices**([_vertices_]) [<>](todo)
...
[#](#densityPlotDensityDoc) *ternaryPlot*.**invert**(_coordinates_) [<>](todo)

Computes ternary values from `[x, y]` coordinates that are scaled by the radius. Unlike the *barycentric*.[invert()](#baryCentricInvertDoc) method this method takes the plot radius into account. Note that for inverting mouse positions, the ternary plot should centered at the origin of the containing SVG element.

[#](#tod) *ternaryPlot*.**convert**(_data_) [<>](todo)

Computes `[x, y]` coordinates that are scaled by the plot radius from ternary data. Unlike the *barycentric*.[convert()](#baryCentricInvertDoc) method, this method takes the plot radius into account.

[#](#ternaryPlotVerticesDoc) *ternaryPlot*.**vertices**(_vertices_) [<>](todo)

If _vertices_ is specified, removes the radius scale from _vertices_ and sets the vertices of the *barycentric()* function passed to *ternaryPlot()*. If _vertices_ is not specified, return the current scaled vertices.

[#](#ternaryPlotRadiusDoc) *ternaryPlot*.**radius**([_radius_]) [<>](todo)
...
[#](#ternaryPlotDomainsDoc) *ternaryPlot*.**domains**([_domains_]) [<>](todo)
...
To set domains without these extra checks use *ternaryPlot*.[setDomains(_domains_)](#ternaryPlotSetDomains).

[#](#ternaryPlotLabelsDoc) *ternaryPlot*.**labels**([_labels_]) [<>](todo)
...
[#](#ternaryPlotLabelAnglesDoc) *ternaryPlot*.**labelAngles**([_angles_]) [<>](todo)
...
[#](#ternaryPlotLabelOffsetsDoc) *ternaryPlot*.**labelOffsets**([_offsets_]) [<>](todo)
...
[#](#ternaryPlotGridLineCountsDoc) *ternaryPlot*.**gridLineCounts**([_counts_]) [<>](todo)
...
[#](#ternaryPlotTickCountsDoc) *ternaryPlot*.**tickCounts**([_counts_]) [<>](todo)
...
[#](#ternaryPlotTickAnglesDoc) *ternaryPlot*.**tickAngles**([_angles_]) [<>](todo)
...
[#](#ternaryPlotTickAnchorsDoc) *ternaryPlot*.**tickAnchors**([_textAnchors_]) [<>](todo)
...
[#](#ternaryPlotTickSizesDoc) *ternaryPlot*.**tickSizes**([_sizes_]) [<>](todo)
...
[#](#ternaryPlotTickFormatDoc) *ternaryPlot*.**tickFormat**([_format_]) [<>](todo)
...
[#](#ternaryPlotTranslateDoc) *ternaryPlot*.**translate**([scale]) [<>](todo)
...
[#](#ternaryPlotScaleDoc) *ternaryPlot*.**scale**(_[scale]_) [<>](todo)
...
[#](#ternaryPlotGridLinesDoc) *ternaryPlot*.**gridLines**() [<>](todo)
...
Grid lines are generated using [d3.*scale*.ticks()](https://github.com/d3/d3-scale/blob/master/README.md#continuous_ticks). The specified count is only a **hint**; the scale may return more or fewer values depending on the domain.

[#](#ternaryPlotTicksDoc) *ternaryPlot*.**ticks**() [<>](todo)
...
Ticks are generated using [d3.*scale*.ticks()](https://github.com/d3/d3-scale/blob/master/README.md#continuous_ticks). The specified count is only a **hint**; the scale may return more or fewer values depending on the domain.

[#](#ternaryPlotAxisLabelsDoc) *ternaryPlot*.**axisLabels**(_[{center: false}]_) [<>](todo)
...
[#](#ternaryPlotTriangleDoc) *ternaryPlot*.**triangle**() [<>](todo)
...
[#](#ternaryPlotTransformDoc) *ternaryPlot*.**transform**() [<>](todo)
...
[#](#ternaryPlotReverseVerticesDoc) *ternaryPlot*.**reverseVertices**() [<>](todo)

Swaps vertices so reversed domains are displayed correctly. *ternaryPlot*.[domains()](#ternaryPlotDomainsDoc) checks wether the domains are reversed and calls this method if so. You'll rarely need to call this method directly.

[#](#ternaryPlotTransformFromDomainsDoc) *ternaryPlot*.**transformFromDomains**(_domains_) [<>](todo)
...
[#](#ternaryPlotDomainsFromVerticesDoc) *ternaryPlot*.**domainsFromVertices**() [<>](todo)
...
[#](#ternaryPlotSetDomainsDoc) *ternaryPlot*.**setDomains**(domains) [<>](todo)

Sets the domains in order `[A, B, C]`. This method is needed when setting domains that are returned by *ternaryPlot*.domainsFromVertices() while zooming and panning. Usually *ternaryPlot.*[domains()](#ternaryPlotDomainsDoc) is the preferred method for setting domains.
## Future work

* The transform method is hacky, bug-prone piece of work
* Canvas support
