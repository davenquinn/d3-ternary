...
const getDomainLengths = domains => new Set(domains.map(domain => {
  ...
  Math.round((domain[0] + Number.EPSILON) * 100) / 100;
  const d1 = Math.round((domain[1] + Number.EPSILON) * 100) / 100;
  return Math.abs(d1 - d1);
}));

const insideDomain = n => n > 0.999999 ? 1 : n < 0.000001 ? 0 : n; // https://en.wikipedia.org/wiki/Distance_between_two_parallel_lines


const parallelLinesDistance = (b1, b2, m) => (b2 - b1) * Math.sign(b1) / Math.sqrt(m ** 2 + 1); // using sign() is hacky tho
...
function ternaryPlot(barycentric) {
  ...
  function ternaryPlot() {// what should go in here though?
  }

  let [svA, svB, svC] = scaleVertices(); // axes configurations
  ...
  let A = {
    ...
    labelAngle: 0,
    labelOffset: 45,
    // 🤔💭 or relative to radius? labelOffset: radius / 10
    gridLine: lineBetween(svC, svA),
    gridLineCount: 20,
    scale: linear$1().domain([0, 1]),
    tickAngle: 0,
    tickCount: 10,
    tickSize: 6,
  ...
  function scaleVertices(newScaledVertices) {
    ...
    const scaledVertices = barycentric.vertices().map(([x, y]) => [x * radius, y * radius]);
  ...
  ternaryPlot.axisLabels = function ({
    ...
    return [A, B, C].map(d => {
      ...
      const position = [x / radius * (radius + d.labelOffset), y / radius * (radius + d.labelOffset)];
      return {
        position,
        label,
        angle: labelAngle
  ...
  ternaryPlot.domains = function (domains) {
    ...
    if (isReverseDomain) {
      ternaryPlot.reverseVertices(domains);
    } else {
      reverse = false; // in case domains switch from reverse to normal
    }

    ternaryPlot.setDomains(domains); // Get transform corresponding to this domain

    const {
      x,
      y,
      k
    } = ternaryPlot.transformFromDomains(domains); // Set and apply transform ie update barycentric coord system
  ...
  ternaryPlot.gridLineCounts = function (_) {
    return arguments.length ? Array.isArray(_) ? (A.gridLineCount = _[0], B.gridLineCount = _[1], C.gridLineCount = _[2], ternaryPlot) : ((A.gridLineCount = +_, B.gridLineCount = +_, C.gridLineCount = +_), ternaryPlot) : [A.gridLineCount, B.gridLineCount, C.gridLineCount];
  };

  ternaryPlot.ticks = function () {
    return [A, B, C].map(axis => {
      const tickValues = axis.scale.ticks(axis.tickCount);
      const format = typeof tickFormat === "function" ? tickFormat : axis.scale.tickFormat(axis.tickCount, tickFormat);
  ...
  ternaryPlot.tickCounts = function (_) {
    return arguments.length ? Array.isArray(_) ? (A.tickCount = _[0], B.tickCount = _[1], C.tickCount = _[2], ternaryPlot) : ((A.tickCount = +_, B.tickCount = +_, C.tickCount = +_), ternaryPlot) : [A.tickCount, B.tickCount, C.tickCount];
  };
  ...
  ternaryPlot.labelOffsets = function (_) {
    return arguments.length ? (A.labelOffset = _[0], B.labelOffset = _[1], C.labelOffset = _[2], ternaryPlot) : [A.labelOffset, B.labelOffset, C.labelOffset];
  };
  ...
  ternaryPlot.convert = function (_) {
  ...
  ternaryPlot.transform = function () {
    ...
    const [newvA, newvB, newvC] = unscaledVertices.map(([vx, vy]) => [vx * k + tx, vy * k + ty]); // these are the newly transformed vertices BEFORE checking if they within bounds of original triangle

    const mAB = getSlope(vA, vB),
          mAC = getSlope(vA, vC),
          mBC = getSlope(vB, vC); // y-intercepts of zoomed triangle sides
    ...
    const transformedVertices = unscaledVertices.map(([vx, vy]) => [vx * k + tx, vy * k + ty]);
  ...
  ternaryPlot.transformFromDomains = function (domains) {
    ...
    const untranslatedDomainStart = domainFromScale(k); // const untranslatedDomain = [domainFromScale(k), 1 - 2 * domainFromScale(k)]
    ...
    const [tx, ty] = [uvA[0] * shiftA + uvB[0] * shiftB + uvC[0] * shiftC, uvA[1] * shiftA + uvB[1] * shiftB + uvC[1] * shiftC].map(d => d * transform.k);
    transform.x = tx;
    transform.y = ty;
    return transform;
...
function barycentric() {
  const rad = PI / 180; // let normalizeData = true;
  // accessors
  ...
  let [vA, vB, vC] = angles.map(d => [cos$1(d * rad), sin$1(d * rad)]); // default vertices
  ...
  const barycentric = function (d) {
    ...
    return [vA[0] * dA + vB[0] * dB + vC[0] * dC, vA[1] * dA + vB[1] * dB + vC[1] * dC];
  ...
  barycentric.vertices = function (ABC) {
    return arguments.length ? (vA = ABC[0], vB = ABC[1], vC = ABC[2], barycentric) : [vA, vB, vC];
  };

  return barycentric;
}

const axisLabels = (g, labels) => g.selectAll("text").data(labels, d => d.label).join(enter => enter.append("text").attr("dy", (d, i) => i === 2 ? "-0.5em" : ".5em").attr("transform", (d, i) => `translate(${d.position})rotate(${d.angle})`).attr("text-anchor", "middle").text(d => d.label), update => update.attr("transform", (d, i) => `translate(${d.position})rotate(${d.angle})`));
/*  
...
const ticks$1 = g => g.selectAll("g").data(d => d, d => d.tick).join(enter => {
  const tickGroups = enter.append("g").attr("class", "tick").attr("transform", d => `translate(${d.position})`);
  tickGroups.append("text").attr("text-anchor", d => d.textAnchor).attr("transform", d => `rotate(${d.angle})`).attr("dx", d => (-d.size - 5) * (d.textAnchor === "start" ? -1 : 1)).attr("dy", ".5em").text(d => d.tick);
  tickGroups.append("line").attr("transform", d => `rotate(${d.angle + 90})`).attr("y2", d => d.size * (d.textAnchor === "start" ? -1 : 1)).attr("stroke", "grey");
  return tickGroups;
}, update => update.attr("transform", d => `translate(${d.position})`), exit => exit.attr("opacity", epsilon).remove());
const grid = (g, gridLines) => g.selectAll("path").data(gridLines).join(enter => enter.append("path").attr("d", d => d).attr("stroke", "#e3e3e3").attr("stroke-width", (d, i) => i & 1 ? 1 : 2), update => update.attr("d", d => d) // theres no exit, lines are only drawn upto 'initial' triangle bounds
);

export { axisLabels, barycentric, grid, ternaryPlot, ticks$1 as ticks };
